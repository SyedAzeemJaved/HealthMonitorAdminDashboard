...
type DoctorAssociationProps = {
  patient_id: number;
};

export default function DoctorAssociation() {
  const { id } = useParams();

  const { user, setLoading } = useContext(AuthContext) as UserContextProps;

  const [association, setAssociation] = useState<DoctorAssociationProps>({
    patient_id: 0,
  });

  const [currentDoctorPatients, setCurrentDoctorPatients] = useState<
    PatientProps[]
  >([]);
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    try {
      ...
      const res = await fetch(
        `${constants.ASSOCIATE}/doctor?patient_id=${association.patient_id}&doctor_id=${id}`,
        {
          method: 'POST',
          headers: {
  ...
  const fetchCurrentDoctorPatients = async () => {
    try {
      const res = await fetch(constants.DOCTORS + `/${id}`, {
        method: 'GET',
        headers: {
          accept: 'application/json',
          Authorization: `Bearer ${user.accessToken}`,
      ...
      setCurrentDoctorPatients(response.patients);
    } catch (err: any) {
      fireToast(
        'There seems to be a problem',
        err.message,
              ...
              <div className="relative z-20 bg-transparent dark:bg-meta-4">
                ...
                >
                  ...
                  {allPatients.map((patient: PatientProps) => {
                    const isFound = currentDoctorPatients.find(
