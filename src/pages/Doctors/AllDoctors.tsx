...
export default function AllDoctors() {
  ...
  const handleDeleteClick = async (id: number) => {
    let r = confirm('Are you sure you want to delete this doctor?');
    if (r === true) {
      try {
  ...
  const fetchAllDoctors = async () => {
    try {
      const res = await fetch(
        `${constants.DOCTORS}?page=${pageNumber}&size=${constants.RESULTS_PER_PAGE}`,
        {
          method: 'GET',
          headers: {
            accept: 'application/json',
      ...
      const doctorArr: DoctorProps[] = response.items.map(
        (doctor: DoctorProps) => {
          return {
            id: doctor.id,
            name: doctor.name,
            ...
            additional_details: {
              ...
              blood_group: doctor.additional_details.blood_group,
            },
                      ...
                      <tr key={doctor.id}>
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="text-black dark:text-white">
                            {doctor.additional_details?.phone ??
                              'Not specified'}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <div className="flex items-center space-x-3.5">
                            ...
                            <button
                              className="hover:text-primary"
                              onClick={() => handleAssociatePatients(doctor.id)}
                            >
                              <svg
