...
export default function AllCaretakers() {
  ...
  const [caretakers, setCaretakers] = useState<CaretakerProps[]>([]);
  ...
  const handleAssociatePatients = (id: number) => {
    navigate(`/caretakers/associate/${id}`);
  };
  ...
  const handleDeleteClick = async (id: number) => {
    let r = confirm('Are you sure you want to delete this caretaker?');
    if (r === true) {
      try {
  ...
  const fetchAllCaretakers = async () => {
    try {
      const res = await fetch(
        `${constants.CARETAKERS}?page=${pageNumber}&size=${constants.RESULTS_PER_PAGE}`,
        {
          method: 'GET',
          headers: {
            accept: 'application/json',
      ...
      const caretakerArr: CaretakerProps[] = response.items.map(
        (caretaker: CaretakerProps) => {
          return {
            id: caretaker.id,
            name: caretaker.name,
  ...
  return (
    <>
      <Breadcrumb pageName="Caretakers" />
                    ...
                    return (
                      <tr key={caretaker.id}>
                        <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                          ...
                          <p className="text-sm">{caretaker.email}</p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="text-black dark:text-white">
                            {caretaker.additional_details?.phone ??
                              'Not specified'}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <div className="flex text-black dark:text-white">
                            {caretaker.patients.length > 0 ? (
                              caretaker.patients.map((patient) => (
                                <p
                                  key={patient.id}
                                  className="mr-2 rounded-2xl bg-stroke p-2 text-sm hover:bg-opacity-75 dark:bg-primary hover:dark:bg-opacity-75"
                                >
                                  {patient.name}
                                </p>
                              ))
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <div className="flex items-center space-x-3.5">
                            <button
                              ...
                              onClick={() => handleEditClick(caretaker.id)}
                            >
                              <svg
                            ...
                            <button
                              className="hover:text-primary"
                              onClick={() =>
                                handleAssociatePatients(caretaker.id)
                              }
                            >
                              <svg
                            ...
                            <button
                              className="hover:text-primary"
                              onClick={() => handleChangePassword(caretaker.id)}
                            >
                              <svg
                                className="fill-tranparent stroke-current stroke-[1.5px]"
                                width="18"
                            ...
                            <button
                              className="hover:text-primary"
                              onClick={() => handleDeleteClick(caretaker?.id)}
                            >
                              <svg
                                className="fill-current"
                                width="18"
