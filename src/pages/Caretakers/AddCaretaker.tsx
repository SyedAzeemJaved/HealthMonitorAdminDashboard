...
export default function AddCaretaker() {
  ...
  const [caretaker, setCaretaker] = useState<CreateCaretakerProps>({
    name: '',
    email: '',
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    try {
      ...
      if (!caretaker.name || !caretaker.email || !caretaker.password) {
        throw new Error('Please fill all input fields');
      }

      if (caretaker.password !== caretaker.confirm_password) {
  ...
  return (
    <>
      <Breadcrumb pageName="Add Caretaker" />
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
          <h3 className="font-medium text-black dark:text-white">
            Add a new caretaker to HealthMate
          </h3>
        </div>
        <form action="#">
          <div className="p-6.5">
