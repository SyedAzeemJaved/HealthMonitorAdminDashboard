...
export default function AllPatients() {
  ...
  const handleDeleteClick = async (id: number) => {
    let r = confirm('Are you sure you want to delete this patient?');
    if (r === true) {
      try {
  ...
  const fetchAllPatients = async () => {
    try {
      const res = await fetch(
        `${constants.PATIENTS}?page=${pageNumber}&size=${constants.RESULTS_PER_PAGE}`,
        {
          method: 'GET',
          headers: {
            accept: 'application/json',
        ...
        <span
          ...
          onClick={() => {
            (async () => {
              try {
                const res = await fetch(
                  `${constants.DISASSOCIATE}/${type}?patient_id=${patient_id}&${type}_id=${id}`,
                  {
                    method: 'POST',
                    headers: {
                      accept: 'application/json',
                      ...
                      <tr key={patient.id}>
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="text-black dark:text-white">
                            {patient.additional_details?.phone ??
                              'Not specified'}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <div className="flex text-black dark:text-white">
                            {patient.caretakers.length > 0 ? (
                              patient.caretakers.map((caretaker) =>
                                renderPill({
                                  id: caretaker.id,
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <div className="flex items-center space-x-3.5">
                            ...
                            <button
                              className="hover:text-primary"
                              onClick={() => handleChangePassword(patient.id)}
                            >
                              <svg
